...
def cariData():
    os.system('cls')
    cari_judul = input("Masukkan kategori buku yang ingin dicari: ") 
    df = pd.read_csv('buku.csv') 
    filtered_df = df[df['judul'] == cari_judul]  
    ...
    if not filtered_df.empty: 
        ...
        print(f"Tidak ada buku dengan kategori '{cari_judul}'")  
...
def terimaRequest():
    ...
    if not filtered_requests.empty: 
        filtered_requests_display = filtered_requests.reset_index(drop=True)
        filtered_requests_display.index += 1
        ...
        try:
            ...
            if 1 <= request_index <= len(filtered_requests): 
                ...
                request_df.loc[approved_request_index, 'tanggal peminjaman'] = date.today()
    ...
    if not return_requests.empty:
        ...
        try:
            ...
            if 1 <= return_index <= len(return_requests): 
                ...
                today = date.today() + pd.Timedelta(days=13)
...
def visitor_menu():
    ...
    while True:
        ...
        if menu == "1":
            df = cariData() # Memanggil fungsi searchCategory() untuk mencari buku berdasarkan kategori
            if df is not None:
...
def awal():
    ...
    user_type = input("[1. Menu Pengunjung][2. Menu Admin][3. Keluar]= ")
